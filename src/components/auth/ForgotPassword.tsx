  ...
  const handleVerificationAndPasswordResetSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data.status === 1) {
        // Assuming status 1 means success
          ...
          <div className="space-y-[32px]">
            ...
            {step === 1 && (
              <form onSubmit={handleEmailSubmit}>
                ...
                {message && <p className={messageType === 'error' ? 'text-red-500 font-bold mt-2' : 'text-green-500 mt-2'}>{message}</p>}
              </form>
            )}
            ...
            <p className="mt-4 w-full text-center text-gray-600 text-sm">
              ...
              <a href="/login" className="font-bold text-[#6941C6]">
                {' '}
                Log in
              </a>
            </p>
